...
pub struct ChildListIterator<'list, O: Object> {
    list: &'list ChildList<O>,
    idx: usize
}

impl<'list, O: Object> IntoIterator for &'list ChildList<O> {

    type Item = Ptr<O>;
    type IntoIter = ChildListIterator<'list, O>;
...
impl<'list, O: Object> Iterator for ChildListIterator<'list, O> {

    type Item = Ptr<O>;

    fn next(&mut self) -> Option<Self::Item> {
        if self.idx >= self.list.n_children() {
            return None;
        }
        self.idx += 1;
        Some(self.list.children[self.idx - 1].ptr())
    }
